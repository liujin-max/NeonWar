...
{
    public GameObject Load(string bundleName, string name, Vector3 position, Transform parent)
    {
        // var obj = Instantiate(Resources.Load<GameObject>(path), position, Quaternion.identity, parent);
        var obj = Instantiate(AssetsManager.LoadPrefab(bundleName, name), position, Quaternion.identity, parent);
