    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				#if FISHEYE_ON
				half2 midF = half2(0.5 *  _MainTex_ST.x, 0.5 *  _MainTex_ST.y);
				half2 dF = i.uv - midF;
				half fishInt = (2.0 * 3.141592 / (2.0 * sqrt(dot(midF, midF)))) * (_FishEyeUvAmount + 0.001);
				half bind = sqrt(dot(midF, midF));
				i.uv = midF + normalize(dF) * tan(sqrt(dot(dF, dF)) * fishInt) * bind / tan(bind * fishInt);
				#endif

				#if PINCH_ON
				half2 midP = half2(0.5 *  _MainTex_ST.x, 0.5 *  _MainTex_ST.y);
				half2 dP = i.uv - midP;
				half pinchInt = (2.0 * 3.141592 / (2.0 * sqrt(dot(midP, midP)))) * (-_PinchUvAmount + 0.001);
